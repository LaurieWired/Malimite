...
public class ProjectWindow {
    ...
    public static void setupWindow(Stage stage, File file, AppConfig appConfig, VBox root, Project malimiteProject) {
        ...
        terminalLabel.getStyleClass().add("terminal_label");
        consoleArea.setTop(terminalLabel);
        consoleArea.setCenter(console);
    
        populateLefthandTree(resourcesRoot, file);
    
        SplitPane mainSplitPane = new SplitPane();
        mainSplitPane.setOrientation(Orientation.VERTICAL);
        mainSplitPane.getItems().addAll(splitPane, consoleArea);
        mainSplitPane.setDividerPositions(0.8); // You can adjust this value as needed
    
        // Ensure the mainSplitPane takes up the majority of the space
        VBox.setVgrow(mainSplitPane, Priority.ALWAYS);
    
        // Add all components to the root VBox
        root.getChildren().addAll(menuContainer, menuSeparator, mainSplitPane);

        initializeProject(malimiteProject, file);
    }

    private static void initializeProject(Project malimiteProject, File file) {
        // Now initialize the actual project if we have a valid bundle file (has an Info.plist)
        if (!infoPlistPath.isEmpty()) {
            System.out.println("Opening Malimite project");

            // Run in the background so it doesn't block the UI
            ...
            thread.setDaemon(true); // Mark the thread as a daemon thread so it doesn't prevent application exit
            thread.start();
        } else {
        ...
        if (currentFilePath != null) {
            try {
                ...
                if (zipEntryPath.endsWith("plist") && PlistUtils.isBinaryPlist(contentBytes)) {
                    ...
                    //this.fileContentArea.setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_JSON); //TODO
                } else {
